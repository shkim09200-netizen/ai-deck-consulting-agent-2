...
export function scriptSystem(opts: {
  ...
  return [
    ...
    "- 각 섹션을 beats(대사 줄) 배열로 나눈다. 슬라이드 분할은 다음 단계(스켈레톤)가 내용 단위로 정하므로 click 같은 전환 마커는 쓰지 않는다.",
    "",
...
export function skeletonSystem(opts: { sectorGuide?: string; customStructure?: SectionDef[]; directives?: string }): string {
  ...
  return [
    ...
    "",
    "[슬라이드 분할 규칙]",
    "- 스크립트를 '한 슬라이드 = 한 메시지' 원칙으로 내용 단위로 나눈다. (click 마커는 없다.)",
    "- 한 문장이라도 따로 강조하면 좋은 부분은 별도 슬라이드로 분리한다.",
    ...
    "- speakerNotes는 한 문단으로 붙여 쓰지 말고, 문장마다 줄바꿈(\\n)으로 나눠 여러 줄로 쓴다 (읽고 발표하기 쉽게).",
    "",
    "[이미지/자료 지시]",
